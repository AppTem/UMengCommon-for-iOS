...
#复制UMengCommon.framework到打包目录
FRAMEWORKPATH="$PROJECT_DIR"/../UMengCommonSDK/UMengCommon.embeddedframework/UMengCommon.framework
cp -rf "$FRAMEWORKPATH" "$CONFIGURATION_BUILD_DIR"

#复制友盟社会化组件所需要的资源
cp -rf "$PROJECT_DIR"/../UMengCommonSDK/UMengCommon.embeddedframework/Resources/* "$CONFIGURATION_BUILD_DIR"


NIBFILES=""

pushd "$PROJECT_DIR"/../UMengCommonSDK/UMengCommon.embeddedframework/Resources
for filename in *
do
NIBFILES="${NIBFILES} "$filename
done
popd
...
"$AIR_SDK_PATH"/bin/adt -package -target ane "$TARGET_NAME" extension.xml -swc "$EXTENSION_SWC_FILE_NAME" -platform default library.swf -platform iPhone-ARM -platformoptions platformoptions.xml "$NATIVE_EXTENSION_STATIC_LIB_NAME" library.swf UMengCommon.framework ${NIBFILES}
popd

cp -f "$CONFIGURATION_BUILD_DIR"/"$TARGET_NAME" "$PROJECT_DIR"/../FlexANE/DemoUMeng/libs
